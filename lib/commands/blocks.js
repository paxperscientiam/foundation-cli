...
function listBuildingBlocks(args, options) {
  fetchUrl('http://foundation.zurb.com', '/building-blocks/data/building-blocks.json')
    .then(function(result) {
      _.each(result.body, function(value, key) {
...
function installBuildingBlock(args, options) {
  var name = args[0];
...
function installFiles(name) {
  var path = '/building-blocks/files/building-blocks/' + name + '/' + filename;
  fetchUrl('http://foundation.zurb.com', path)
    .then(function(file) {
      console.log(file);
    })

  // async.map(FILETYPES, function(filetype) {
  ...
  //         // if(file.length > 0) {
  //         //   fs.writeFile(filetype.directory + filetype.prefix + filename, file);
  //         // }
  //       });
  //   });
  // });
...
function downloadZipFile(name, callback) {
  ...
  fetchUrl('http://foundation.zurb.com', path)
    .then(function (file) {
      if (Buffer.byteLength(file.body,'utf8') > 0) {
        fs.writeFile(filename, file.body);
        console.log(filename + " saved.");
      } else {
        console.log(filename + " is zero sized.");
      }
    })
    .catch(function (err) {
